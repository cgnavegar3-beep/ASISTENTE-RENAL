# v. 02 mar 2026 20:15 (Corrección final: Títulos alineados a la izquierda)
            ...
            with st.spinner("Procesando análisis clínico..."):
                ...
                try:
                    ...
                    with placeholder_salida.container():
                        # --- MODIFICACIÓN DE ESTRUCTURA Y TÍTULOS (IZQUIERDA) ---
                        st.markdown(f'<div class="synthesis-box {glow}">{sintesis}</div>', unsafe_allow_html=True)
                        st.markdown("---")
                        # Título BLOQUE 2 centrado
                        st.markdown(f'<div style="text-align:left; font-weight:700; margin-bottom: 10px;">BLOQUE 2: TABLA COMPARATIVA</div>', unsafe_allow_html=True)
