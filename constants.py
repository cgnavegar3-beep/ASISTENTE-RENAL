# v. 09 mar 2026 20:45 (CONTROL DE INTEGRIDAD TOTAL: CÓDIGO ÍNTEGRO - BOTÓN CENTRADO + ESPEJO DATOS)
...
def extraer_datos_resumen(html_tabla):
    ...
    # FIX: Recalcular TOTAL AFECTADOS como suma de riesgos para evitar duplicados del prompt
...
def inject_styles():
    ...
    .warning-yellow { background-color: #fff9db; color: #856404; padding: 20px; border-radius: 10px; border: 1px solid #f9f9c5; margin-top: 20px; text-align: center; font-size: 0.85rem; line-height: 1.5; }
    ...
    /* Estilo del botón de guardado corregido */
    .save-btn-container { display: flex; justify-content: center; margin-bottom: 20px; }
    div.stButton > button[key="btn_grabar_real"] {
...
st.markdown('<div class="sub-version">v. 09 mar 2026 20:45</div>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    # --- BOTÓN DE GRABADO (CENTRADO Y PARPADEANTE) ---
    if st.session_state.resultado_ia:
        st.markdown('<div class="save-btn-container">', unsafe_allow_html=True)
        if st.button("📥 GRABAR DATOS EN NUBE", key="btn_grabar_real"):
            try:
                ...
                st.success("✅ Datos grabados exitosamente.")
            except Exception as e: st.error(f"Error en grabado: {e}")
    ...
    st.markdown(f'<div style="text-align:right; font-size:0.6rem; color:#ccc; font-family:monospace; margin-top:10px;">v. 09 mar 2026 20:45</div>', unsafe_allow_html=True)
...
with tabs[2]:
    ...
    with sub_tabs[0]:
        ...
        except: st.info("Conectando con la base de datos de Validaciones...")

    with sub_tabs[1]:
        try:
            df_ic = conn.read(worksheet="INTERCONSULTAS")
            st.dataframe(df_ic, use_container_width=True, hide_index=True)
        except: st.info("Conectando con la base de datos de Interconsultas...")

    with sub_tabs[2]:
        st.write("Sección de histórico general de actividad.")
