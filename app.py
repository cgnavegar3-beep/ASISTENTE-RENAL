# v. 20 mar 2026 08:45 (CORRECCIÓN VOLCADO TABLA 1)
...
def sanitizar_para_gsheets(valor):
    """Convierte tipos de Numpy/Pandas a tipos nativos de Python aceptados por gspread."""
    if pd.isna(valor):
        return ""
    if hasattr(valor, "item"): # Maneja tipos numpy (int64, float64)
        return valor.item()
    return valor
...
def guardar_en_google_sheets(df_val_actual, df_meds_actual):
    try:
        ...
        if id_actual not in ids_existentes:
            # Seleccionar última fila del ID actual
            subset_val = df_val_actual[df_val_actual["ID_REGISTRO"] == id_actual]
            if not subset_val.empty:
                fila_val_raw = subset_val.iloc[-1].to_dict()
                # Sanitización estricta de la fila
                fila_val_limpia = [sanitizar_para_gsheets(v) for v in fila_val_raw.values()]
                ws_val.append_row(fila_val_limpia)
 
        # --- TABLA 2: MEDICAMENTOS ---
        ws_meds = doc.worksheet("MEDICAMENTOS")
        data_meds_nube = ws_meds.get_all_records()
        df_nube_meds = pd.DataFrame(data_meds_nube)
        meds_a_procesar = df_meds_actual[df_meds_actual["ID_REGISTRO"] == id_actual]
        ...
        for _, fila in meds_a_procesar.iterrows():
            ...
            if not ya_existe:
                # Sanitización estricta de la fila de medicamento
                fila_conv = [sanitizar_para_gsheets(v) for v in fila.values.tolist()]
...
st.markdown('<div class="sub-version">v. 20 mar 2026 08:45</div>', unsafe_allow_html=True)
...
st.markdown(f"""<div class="warning-yellow">⚠️ <b>Apoyo a la revisión farmacoterapéutica. Verifique fuentes oficiales.</b></div> <div style="text-align:right; font-size:0.6rem; color:#ccc; font-family:monospace; margin-top:10px;">v. 20 mar 2026 08:45</div>""", unsafe_allow_html=True)
