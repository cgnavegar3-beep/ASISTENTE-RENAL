# v. 20 mar 2026 08:15 (INTEGRACIÓN TEST VOLCADO)
...
# --- FUNCIÓN INTEGRADA (ENTREGA 2) ---
def test_volcado_simple(df_val_actual):
    try:
        doc = conectar_google_sheets()
        ws = doc.worksheet("VALIDACIONES_TEST")

        if df_val_actual.empty:
            st.error("DF vacío")
            return
        ...
        st.write("Fila final:", fila_final)
        ws.append_row(fila_final)
        st.success("✅ TEST VOLCADO OK")
...
st.markdown('<div class="sub-version">v. 20 mar 2026 08:15</div>', unsafe_allow_html=True)
...
with tabs[2]:
    ...
    # --- INTEGRACIÓN BOTÓN DE LLAMADA (ENTREGA 3) ---
    st.button("TEST VALIDACIONES", on_click=lambda: test_volcado_simple(st.session_state.df_val))
...
# Pie de página (Requisito 7: Versionado Local)
st.markdown(f"""<div style="position: fixed; bottom: 10px; right: 15px; font-size: 0.6rem; color: #555; font-family: monospace;">v. 20 mar 2026 08:15 | ASISTENTE RENAL</div>""", unsafe_allow_html=True)

# Aviso Legal Inferior (Requisito 4: Blindaje)
st.markdown("""<div class="warning-yellow">⚠️ <b>ADVERTENCIA LEGAL:</b> Esta herramienta es un apoyo a la decisión clínica basado en IA y fórmulas farmacocinéticas. No sustituye el juicio profesional del farmacéutico o médico. Verifique siempre los resultados con la ficha técnica oficial (AEMPS).</div>""", unsafe_allow_html=True)
