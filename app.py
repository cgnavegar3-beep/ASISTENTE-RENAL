# v. 23 mar 2026 13:15 (EVOLUCIÓN: REPARACIÓN FILTROS DASHBOARD)
...
def sincronizar_desde_nube():
    try:
        ...
        st.session_state.toast("✅ Nube sincronizada (Valores Numéricos OK)", icon="🔄")
...
with tabs[3]:
  st.markdown("### 📈 Dashboard de Gestión Renal")
   ...
   if not df_v_dash.empty:
       ...
       # QUIRÚRGICO: Sincronizar medicamentos con el filtro de pacientes
       df_m_dash = df_m_dash[df_m_dash["ID_REGISTRO"].isin(df_filtered_val["ID_REGISTRO"])]
           ...
           if not df_m_dash.empty:
              df_alertas = df_m_dash[pd.to_numeric(df_m_dash["NIVEL_ADE_CG"], errors='coerce') > 0]
               if not df_alertas.empty:
                  ...
                  df_top_raw = df_alertas.groupby("MEDICAMENTO").size().reset_index(name='count').sort_values(by="count", ascending=False)
                  df_top_final = df_top_raw.head(5) # QUIRÚRGICO: Garantizar Top 5 real
