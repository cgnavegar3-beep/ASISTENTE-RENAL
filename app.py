...
def obtener_modelos_vivos():
    """Detecta qué modelos responden realmente en tu cuenta"""
    try:
...
def llamar_ia_en_cascada(prompt, imagen=None):
    """Prioridad: 2.5-flash -> 1.5-pro -> Otros"""
    disponibles = obtener_modelos_vivos()
    # Tu orden de preferencia
    preferencia = ['2.5-flash', '1.5-pro', '2.0-flash-exp', '1.5-flash']
    
    # Creamos la lista final: primero tu orden, luego el resto que esté vivo
    modelos_a_intentar = [m for m in preferencia if m in disponibles]
    for m in disponibles:
        if m not in modelos_a_intentar:
            modelos_a_intentar.append(m)
    
    if not modelos_a_intentar: modelos_a_intentar = ['2.5-flash']
    ...
    for mod_name in modelos_a_intentar:
        try:
            ...
            continue
    return "⚠️ Error: Ningún modelo respondió."

# --- 1. CONFIGURACIÓN Y ESTILOS (INTERFAZ SAGRADA) ---
...
def inject_ui_styles():
    ...
    .version-tag {
        ...
        font-size: 0.6rem; color: #ccc; font-family: monospace; z-index: 100;
    }
    </style>
    <div class="version-tag">v. 16 feb 20:38</div>
    """
...
def analizar_y_volcar(imagen):
    prompt = "Extrae exclusivamente la lista de fármacos y dosis de esta imagen médica."
    lectura = llamar_ia_en_cascada(prompt, imagen)
    if not es_seguro_rgpd(lectura):
        st.session_state.meds_content = "⚠️ BLOQUEO: Se han detectado datos personales."
    else:
    ...
    with col_der:
        ...
        with c_btn:
            ...
            if pasted.image_data is not None:
                # Blindaje: Solo procesa si hay datos reales
                try:
                    ...
                    st.toast("⚠️ Portapapeles vacío o no reconocido.")
            elif st.session_state.get('p_btn_clicked'): # Feedback si está vacío
                st.toast("⚠️ No hay imagen en el portapapeles.")
    ...
    with b_val:
        if st.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True):
            if st.session_state.meds_content:
                with st.spinner("Consultando seguridad renal..."):
                    p_val = f"Analiza riesgos con FG {valor_fg} mL/min para: {st.session_state.meds_content}"
                    res = llamar_ia_en_cascada(p_val)
                    st.info(res)
            else:
