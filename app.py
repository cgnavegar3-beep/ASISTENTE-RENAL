...
def llamar_ia_en_cascada(prompt, imagen=None):
    prioridad = ['1.5-pro', '2.5-pro', '2.5-flash', '1.5-flash']
...
def analizar_y_volcar(imagen):
    prompt = "Actúa como OCR médico. Extrae lista de fármacos y dosis. Si ves datos personales, inclúyelos para el filtro."
...
with tabs[0]:
    ...
    # B) CALCULADORA E INTERFAZ DE CAPTURA
    ...
    with col_der:
        ...
        with c_up:
            archivo = st.file_uploader("Pegar/Subir", label_visibility="collapsed", key="uploader_auto", type=['png', 'jpg', 'jpeg'])
            if archivo:
                img = Image.open(archivo)
                analizar_y_volcar(img)
        with c_btn:
            pasted = paste_image_button(label="✂️ RECORTE", key="p_btn", background_color="#FFFFFF")
    ...
    st.markdown('<div class="rgpd-box"><b>Protección de Datos:</b> Pegue el recorte o suba el archivo para procesar.</div>', unsafe_allow_html=True)
        ...
        if st.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True):
            if st.session_state.meds_content:
                with st.spinner("IA analizando..."):
                    prompt_val = f"Paciente con FG {fg_m if fg_m else fg}. Valida riesgos de estos fármacos: {st.session_state.meds_content}"
