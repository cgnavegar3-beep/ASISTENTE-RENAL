...
# 1. Estructura y función de cuadros negros (ZONA y ACTIVO).
# 2. Título principal y pestañas (Tabs).
# 3. Registro de paciente: estructura, columnas y función.
# 4. Interfaz Dual (Calculadora y FG): diseño y lógica Cockcroft-Gault.
# 5. Estructura de zona de recortes (Uploader + Botón 0.65/0.35).
# 6. Cuadro de listado de medicamentos (TextArea).
# 7. Barra dual de botones (VALIDAR ADECUACIÓN / RESET).
# 8. Aviso con sombra amarillo (Warning inferior).
# =================================================================
...
# --- 1. CONFIGURACIÓN Y ESTILOS (MANTENIENDO BLINDAJE) ---
...
def analizar_y_volcar(imagen):
    prompt = "Extrae lista de fármacos y dosis de esta imagen médica sin preámbulos."
...
st.markdown('<div class="version-display">v. 16 feb 20:55</div>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    # B) CALCULADORA E INTERFAZ DUAL (BLINDADA)
        ...
        with c_btn:
            ...
            if pasted.image_data is not None:
                try:
                    ...
                    st.toast("⚠️ Error al leer imagen.")

    st.write("")
    st.markdown("---")

    # C) MEDICAMENTOS (BLINDADO)
        ...
        if st.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True):
            if st.session_state.meds_content:
                with st.spinner("Analizando..."):
                    res = llamar_ia_en_cascada(f"FG: {valor_fg}. Analiza fármacos: {st.session_state.meds_content}")
                    st.info(res)
    with b_res:
