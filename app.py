...
# v. 25 mar 2026 13:00 (EVO: CONSULTA DINÁMICA RENAL - MOD QUIRÚRGICO)
...
st.markdown('<div class="sub-version">v. 25 mar 2026 13:00</div>', unsafe_allow_html=True)

tabs = st.tabs(["💊 VALIDACIÓN", "📄 INFORME", "📊 DATOS", "📈 GRÁFICOS", "🔍 CONSULTA DINÁMICA RENAL"])
...
# --- ENCAPSULAMIENTO: CONSULTA DINÁMICA RENAL (EVO 25 MAR) ---
with tabs[4]:
    st.markdown("### 🔍 Explorador Renal Avanzado")

    def normalizar_texto_renal(t):
    ...
    source_opt = st.radio("Origen de datos", ["VALIDACIONES", "MEDICAMENTOS"], horizontal=True)
    df_raw = st.session_state["df_sync_val"].copy() if source_opt == "VALIDACIONES" else st.session_state["df_sync_meds"].copy()

    if not df_raw.empty:
        # Capa 0: Normalización Crítica
        for col in df_raw.columns:
            if df_raw[col].dtype == 'object':
                df_raw[col] = df_raw[col].apply(normalizar_texto_renal)
        
        st.markdown("#### 💬 Consulta en Lenguaje Natural")
        user_query = st.text_input("Ej: Pacientes de Marin con edad > 80 y % con Metformina", placeholder="Describa su análisis...")
        
        # Bloque A: Filtros Dinámicos
        with st.expander("🛠️ Cohorte y Filtros Acumulativos", expanded=True):
            col_f1, col_f2 = st.columns([1, 2])
            filtro_col = col_f1.selectbox("Columna", df_raw.columns)
            df_filtered = df_raw.copy()
            
            if df_raw[filtro_col].dtype in ['int64', 'float64']:
                op = col_f2.selectbox("Operador", ["=", ">", "<", ">=", "<=", "Rango"])
                if op == "Rango":
                    v_min, v_max = col_f2.slider("Valores", float(df_raw[filtro_col].min()), float(df_raw[filtro_col].max()), (float(df_raw[filtro_col].min()), float(df_raw[filtro_col].max())))
                    df_filtered = df_raw[df_raw[filtro_col].between(v_min, v_max)]
                else:
                    val = col_f2.number_input("Valor", value=float(df_raw[filtro_col].mean()))
                    if op == "=": df_filtered = df_raw[df_raw[filtro_col] == val]
                    elif op == ">": df_filtered = df_raw[df_raw[filtro_col] > val]
                    elif op == "<": df_filtered = df_raw[df_raw[filtro_col] < val]
                    elif op == ">=": df_filtered = df_raw[df_raw[filtro_col] >= val]
                    elif op == "<=": df_filtered = df_raw[df_raw[filtro_col] <= val]
            else:
                vals = col_f2.multiselect("Selección", df_raw[filtro_col].unique())
                if vals: df_filtered = df_raw[df_raw[filtro_col].isin(vals)]

        # Bloque B: Métricas y Top N
        c_m1, c_m2, c_m3 = st.columns([1, 1, 1])
        metrica = c_m1.selectbox("Métrica", ["Conteo", "Suma", "Promedio", "Máximo", "Mínimo"])
        agrupar = c_m2.selectbox("Agrupar por", [None] + list(df_raw.columns))
        top_n_act = c_m3.checkbox("Activar Top N")
        top_n_val = c_m3.number_input("N", min_value=1, value=5) if top_n_act else None

        # Bloque C: Motor de Salida
        if not df_filtered.empty:
            res = df_filtered
            if agrupar:
                if metrica == "Conteo": res = df_filtered.groupby(agrupar).size().reset_index(name='Total')
                elif metrica == "Suma": res = df_filtered.groupby(agrupar).sum(numeric_only=True).reset_index()
                elif metrica == "Promedio": res = df_filtered.groupby(agrupar).mean(numeric_only=True).reset_index()
            
            if top_n_act:
                res = res.head(top_n_val)

            st.markdown("---")
            out_t1, out_t2 = st.tabs(["📋 Tabla Dinámica", "📊 Visualización"])
            with out_t1:
                st.dataframe(res, use_container_width=True)
            with out_t2:
                tipo_viz = st.segmented_control("Tipo de Gráfico", ["Barras", "Líneas", "Tarta"], default="Barras")
                if agrupar:
                    if tipo_viz == "Barras": fig = px.bar(res, x=agrupar, y=res.columns[-1])
                    elif tipo_viz == "Líneas": fig = px.line(res, x=agrupar, y=res.columns[-1])
                    else: fig = px.pie(res, names=agrupar, values=res.columns[-1])
                    st.plotly_chart(fig, use_container_width=True)
        else:
            st.warning("No hay datos que coincidan con los filtros.")
    else:
        st.info("Sincronice los datos en la pestaña DATOS para comenzar el análisis.")
...
st.markdown(f'<div style="text-align: right; font-size: 0.6rem; color: #ccc; font-family: monospace;">v. 25 mar 2026 13:00</div>', unsafe_allow_html=True)
