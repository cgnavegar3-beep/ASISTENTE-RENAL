...
def run_ia_task(prompt, image_bytes=None):
    for i in range(len(MODELS)):
        ...
        try:
            ...
            continue
    st.session_state.active_model_idx = 0
    return "ERROR: Agotados todos los intentos."

# --- 3. ESTILOS CSS (ESTILO EXCEL CON BARRA INFERIOR) ---
@st.cache_resource
def load_vademecum():
    try:
...
def inject_ui_styles():
    ...
    style_html = """
    <style>
        /* Cuadro de versión superior izquierdo */
        .discreet-v {
        ...
        /* --- ESTILO PESTAÑAS TIPO EXCEL --- */
        /* Texto y posición para pestañas activas */
        div[role="tab"]:not([aria-selected="false"]) {
            color: #FF4B4B !important;  /* Rojo distintivo */
            font-weight: bold !important;
            position: relative;
        }

        /* Barra debajo del texto activo (Pseudo-elemento) */
        ...
        /* Texto para pestañas inactivas */
        div[role="tab"][aria-selected="false"] {
            color: #555 !important;
            font-weight: normal !important;
        }

        /* Eliminar estilos por defecto de Streamlit que interfieran */
        button[data-baseweb="tab"] {
            background-color: transparent !important;
            border: none !important;
        }

        /* Caja de Filtrado Glomerular y tarjetas */
        .fg-glow-box {
        ...
        .result-card { padding: 20px; border-radius: 15px; margin-top: 15px; color: #000; }
    </style>
...
def render_tab_validacion(v_text):
    ...
    if img_temp:
        with st.spinner("IA transcribiendo..."):
            ...
            res_ocr = run_ia_task("Extrae nombres y dosis de medicamentos de esta imagen. Solo texto plano.", img_bytes)
