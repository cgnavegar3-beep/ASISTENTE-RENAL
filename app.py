...
with tabs[4]:
    ...
    def reset_consulta_completa():
        # Limpiar filtros dinámicos (lógica de negocio UI)
        st.session_state.filtros_dinamicos = []
        # Limpiar todos los widgets con las claves especificadas
        for key in list(st.session_state.keys()):
            if key.startswith(("f_", "query_", "rk_")):
    ...
    if not df_pool.empty:
        ...
        with st.container(border=True):
            ...
            query_text = st.text_input("Haz una pregunta sobre los datos:", placeholder="Ej: Top 5 medicamentos, Histograma FG,  gráfico pacientes por centro, ¿Cuántos pacientes ...?", key="query_ia_input")
            if query_text:
