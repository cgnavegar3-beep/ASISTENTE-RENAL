...
with tabs[0]:
    ...
    faltan_datos = not all([st.session_state.reg_centro, st.session_state.reg_res, calc_e, calc_p, calc_c, calc_s]) or \
                   st.session_state.reg_res == "-- SELECCIONAR --" or st.session_state.calc_s == "-- SELECCIONAR --" or \
                   (not fg_m and not valor_fg) or \
                   (st.session_state.fgl_mdrd is None) or \
                   (st.session_state.fgl_ckd is None)
    ...
    if not df_pool.empty:
        ...
        with st.container(border=True):
            ...
            for i, filtro in enumerate(st.session_state.filtros_dinamicos):
                ...
                f_id = filtro["id"]
                filtro["col"] = f_c1.selectbox(f"Columna {i+1}", df_pool.columns, key=f"f_col_{f_id}", index=list(df_pool.columns).index(filtro["col"]))
                filtro["op"] = f_c2.selectbox(f"Operador {i+1}", ["== (IGUAL)", "!= (DISTINTO DE)", "> (MAYOR QUE)", "< (MENOR QUE)", "≥ (MAYOR O IGUAL)", "≤ (MENOR O IGUAL)", "contiene"], key=f"f_op_{f_id}")
                
                # Input dinámico según tipo
                if "contiene" in filtro["op"]:
                    filtro["val"] = f_c3.text_input(f"Valor {i+1}", key=f"f_val_{f_id}", value=filtro["val"])
                    ...
                    filtro["val"] = f_c3.number_input(f"Valor {i+1}", key=f"f_val_{f_id}", value=f_val_num)
                else:
                    opciones_unicas = sorted([str(x) for x in df_pool[filtro["col"]].unique() if x])
                    filtro["val"] = f_c3.multiselect(f"Valores {i+1}", opciones_unicas, key=f"f_val_{f_id}")
