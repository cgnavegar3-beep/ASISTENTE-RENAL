...
with tabs[4]:
    ...
    if not df_pool.empty:
        ...
        with st.container(border=True):
            ...
            for i, filtro in enumerate(st.session_state.filtros_dinamicos):
                ...
                if "contiene" in filtro["op"]:
                    filtro["val"] = f_c3.text_input(f"Valor {i+1}", key=f"f_val_{i}", value=filtro["val"], placeholder="elige 1 o varias opciones")
        ...
        agrupar_por = b_col3.selectbox("Segmentar por (Opcional)", ["Ninguno"] + list(df_pool.columns), placeholder="Para ver los resultados separados por la variable seleccionada, AGRUPAR en una o varias categorias.")
