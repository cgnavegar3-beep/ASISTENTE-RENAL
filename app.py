...
def llamar_ia_en_cascada(prompt, imagen=None):
    ...
    for mod_name in modelos_a_intentar:
        try:
            ...
            continue
    return "⚠️ Error: Los modelos están saturados o no hay conexión."
...
def analizar_y_volcar(imagen):
    """Extrae texto de la imagen de forma segura"""
    prompt = "Extrae lista de fármacos y dosis. Si detectas nombres de pacientes o DNI, inclúyelos para activar mi filtro de privacidad."
        ...
        if st.button("🚀 VALIDAR ADECUACIÓN RENAL", use_container_width=True):
            if st.session_state.meds_content:
                with st.spinner("Analizando seguridad..."):
                    p_med = f"Actúa como nefrólogo. FG del paciente: {valor_fg} mL/min. Valida estos fármacos y dosis: {st.session_state.meds_content}. Responde de forma esquemática."
                    respuesta = llamar_ia_en_cascada(p_med)
                    st.info(respuesta)
    with b_res:
