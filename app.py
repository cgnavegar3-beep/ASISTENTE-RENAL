...
# Inicialización de estados con llaves consistentes
...
def inject_ui_styles():
    ...
    .model-badge { background-color: #000000 !important; color: #00FF00 !important; padding: 4px 10px; border-radius: 3px; font-family: monospace !important; font-size: 0.75rem; position: fixed; top: 15px; left: 205px; z-index: 1000000; box-shadow: 0 0 5px #00FF0033; border: 1px solid #00FF00; }
...
st.markdown('<div class="sub-version">v. 22 feb 14:00</div>', unsafe_allow_html=True)
st.markdown('<div class="version-display">v. 22 feb 14:00</div>', unsafe_allow_html=True)
...
with tabs[0]:
    ...
    with c_del:
        st.write(""); st.button("🗑️", help="Limpiar datos del paciente", on_click=reset_registro)
    ...
    if st.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True):
        if txt_meds:
            with st.spinner("Consultando evidencia clínica..."):
                prompt = f"Experto farmacia renal. Analiza FG {valor_fg}: {txt_meds}. INSTRUCCIONES RÍGIDAS: 1. Solo 'Medicamentos afectados:' o 'Fármacos correctamente dosificados'. 2. Lista: [Icono] [Nombre] - [Frase corta]. 3. Inicia bloque técnico con: 'A continuación, se detallan los ajustes de dosis...'"
                ...
                try:
                    ...
                    st.markdown(f'<div class="blue-detail-container">{detalle_clinico.replace("\n", "<br>")}<div class="nota-line">Nota Importante:<br>· Recomendaciones generales. Consulte ficha técnica.</div></div>', unsafe_allow_html=True)
                    
                    # VOLCADO A SESSION STATE
                    ...
                    st.session_state.soip_o = f"Edad: {calc_e} años | Peso: {calc_p}kg | Creatinina: {calc_c}mg/dL | Sexo: {calc_s} | FG: {valor_fg} mL/min"
                    
                    analisis_texto = f"Se identifican los siguientes fármacos con dosis no adecuadas según FG:\n\n{sintesis}\n\nValoración global: riesgo moderado de toxicidad acumulativa; medicación en general compatible."
                    st.session_state.soip_i = analisis_texto
                    st.session_state.soip_p = "Se realiza interconsulta (IC) a su médico de atención primaria (MAP)."
                    
                    st.session_state.ic_motivo = f"Revisión farmacoterapéutica.\n{analisis_texto}\n\nObjetivo: Valoración del MAP sobre ajustes."
                    st.session_state.ic_info = detalle_clinico.replace("A continuación, se detallan los ajustes de dosis para cada fármaco con este valor de FG:", "").strip()
                except: st.info(resp)
...
with tabs[1]:
    ...
    st.text_area("S", value=st.session_state.soip_s, height=70, label_visibility="collapsed", key="s_area")
    
    st.markdown('<div class="linea-discreta-soip">Objetivo (O)</div>', unsafe_allow_html=True)
    st.text_area("O", value=st.session_state.soip_o, height=70, label_visibility="collapsed", key="o_area")
    
    st.markdown('<div class="linea-discreta-soip">Interpretación / Análisis (A)</div>', unsafe_allow_html=True)
    st.text_area("A", value=st.session_state.soip_i, height=130, label_visibility="collapsed", key="a_area")
    
    st.markdown('<div class="linea-discreta-soip">Plan (P)</div>', unsafe_allow_html=True)
    st.text_area("P", value=st.session_state.soip_p, height=70, label_visibility="collapsed", key="p_area")

    st.markdown('<div class="divider-tecnico"></div>')
    ...
    with ic1:
        ...
        st.text_area("Mot", value=st.session_state.ic_motivo, height=200, label_visibility="collapsed", key="mot_area")
    with ic2:
        st.markdown('<div class="linea-discreta-soip">Información Clínica</div>', unsafe_allow_html=True)
        st.text_area("Inf", value=st.session_state.ic_info, height=200, label_visibility="collapsed", key="inf_area")

st.markdown('<div class="warning-yellow">⚠️ Apoyo a la revisión farmacoterapéutica.</div>', unsafe_allow_html=True)
