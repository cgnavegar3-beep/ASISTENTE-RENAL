# v. 19 mar 2026 09:50 (EVOLUCIÓN: MAPEO POR CABECERAS & INTEGRIDAD NUBE)
...
def guardar_en_google_sheets(df_val_actual, df_meds_actual):
    try:
        ...
        # --- EVOLUCIÓN: MAPEO POR CABECERAS PARA VALIDACIONES ---
        ws_val = doc.worksheet("VALIDACIONES")
        headers_val = ws_val.row_values(1)
        ids_existentes = ws_val.col_values(headers_val.index("ID_REGISTRO") + 1 if "ID_REGISTRO" in headers_val else 4)
        
        if id_actual not in ids_existentes:
            fila_df = df_val_actual[df_val_actual["ID_REGISTRO"] == id_actual].iloc[-1].fillna("").to_dict()
            fila_ordenada = []
            for h in headers_val:
                val = fila_df.get(h, "")
                # Limpieza de tipos para compatibilidad JSON API
                if hasattr(val, "item"): val = val.item()
                elif isinstance(val, float) and math.isnan(val): val = ""
                fila_ordenada.append(val)
            ws_val.append_row(fila_ordenada)

        # --- EVOLUCIÓN: MAPEO POR CABECERAS PARA MEDICAMENTOS ---
        ws_meds = doc.worksheet("MEDICAMENTOS")
        ...
        for _, fila in meds_a_procesar.iterrows():
            ya_existe = False
            med_nombre = fila.get("MEDICAMENTO", "")
            if not df_nube_meds.empty and "ID_REGISTRO" in df_nube_meds.columns and "MEDICAMENTO" in df_nube_meds.columns:
            ...
            if not ya_existe:
                ...
                for h in headers_meds:
                    ...
                    fila_ordenada.append(val)
                filas_nuevas.append(fila_ordenada)
...
st.markdown('<div class="sub-version">v. 19 mar 2026 09:50</div>', unsafe_allow_html=True)
...
st.markdown(f"""<div class="warning-yellow">⚠️ <b>Apoyo a la revisión farmacoterapéutica. Verifique fuentes oficiales.</b></div> <div style="text-align:right; font-size:0.6rem; color:#ccc; font-family:monospace; margin-top:10px;">v. 19 mar 2026 09:50</div>""", unsafe_allow_html=True)
