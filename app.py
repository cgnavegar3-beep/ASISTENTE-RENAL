...
def llamar_ia_en_cascada(prompt, imagen=None):
    """Lógica de resiliencia: intenta Pro, luego Flash"""
    prioridad = ['1.5-pro', '2.5-pro', '2.0-flash-exp', '2.5-flash', '1.5-flash']
    ...
    for mod_name in modelos_a_intentar:
        try:
            ...
            continue
    return "⚠️ Error: Los modelos no responden."
...
def procesar_imagen_ia(imagen_pil):
    """Extrae texto de la imagen y lo vuelca al estado de la sesión"""
    with st.spinner("🔍 Analizando imagen médica..."):
        ...
        lectura_ia = llamar_ia_en_cascada(prompt, imagen_pil)
        
        if not es_seguro_rgpd(lectura_ia):
            st.session_state.meds_content = "⚠️ BLOQUEADO: Se detectaron datos personales en la imagen."
        else:
...
with tabs[0]:
    ...
    with col_der:
        ...
        c_up, c_btn = st.columns([0.7, 0.3])
        with c_up:
            # Uploader que acepta pegar archivos
            archivo = st.file_uploader("Subir/Pegar", label_visibility="collapsed", key="uploader_key", type=['png', 'jpg', 'jpeg'])
        with c_btn:
            if st.button("✂️ PROCESAR", use_container_width=True):
                if archivo:
                    img = Image.open(archivo)
                    procesar_imagen_ia(img)
                    st.rerun()
                else:
                    st.warning("Suba una imagen primero.")
    ...
    st.markdown('<div class="rgpd-box"><b>Protección de Datos (RGPD):</b> El sistema bloqueará el texto si detecta identidades.</div>', unsafe_allow_html=True)
    
    # Campo de texto que se actualiza con la IA
    ...
    with b_val:
        if st.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True):
            if "⚠️" in st.session_state.meds_content:
                st.error("BLOQUEADO: Datos personales detectados.")
            elif not st.session_state.meds_content:
                st.warning("El listado está vacío.")
            else:
                with st.spinner("IA analizando seguridad renal..."):
                    prompt_medico = f"Filtrado Glomerular: {fg_m if fg_m else fg} mL/min. Analiza estos fármacos y dosis según guías de ajuste renal: {st.session_state.meds_content}"
                    analisis = llamar_ia_en_cascada(prompt_medico)
                    st.info(analisis)
