...
# (Se asume que c.PROMPT_AFR_V10 y API_KEY están definidos en el entorno o archivo de constantes)
    ...
    if not df_pool.empty:
        ...
        with st.container(border=True):
            ...
            for i, filtro in enumerate(st.session_state.filtros_dinamicos):
                ...
                # Cajetín 1: -- seleccionar --
                opciones_c1 = ["-- seleccionar --"] + list(df_pool.columns)
                idx_c1 = opciones_c1.index(filtro["col"]) if filtro["col"] in opciones_c1 else 0
                filtro["col"] = f_c1.selectbox(f"Columna {i+1}", opciones_c1, key=f"f_col_{fid}", index=idx_c1)
                
                # Cajetín 2: -- seleccionar --
                opciones_c2 = ["-- seleccionar --", "== (IGUAL)", "!= (DISTINTO DE)", "> (MAYOR QUE)", "< (MENOR QUE)", "≥ (MAYOR O IGUAL)", "≤ (MENOR O IGUAL)", "contiene"]
                idx_c2 = opciones_c2.index(filtro["op"]) if filtro["op"] in opciones_c2 else 0
                filtro["op"] = f_c2.selectbox(f"Operador {i+1}", opciones_c2, key=f"f_op_{fid}", index=idx_c2)
                ...
                if filtro["col"] != "-- seleccionar --":
                    if "contiene" in filtro["op"]:
                        ...
                        # Cajetín 3: -- elige 1 o varias opciones --
        ...
        # Variable a medir: -- seleccionar --
        opciones_var = ["-- seleccionar --"] + list(df_pool.columns)
        var_analisis = b_col1.selectbox("Variable a medir", opciones_var, key="query_var")
        
        # Operación: -- seleccionar --
        opciones_ope = ["-- seleccionar --", "Conteo (Total)", "Conteo Único (Pacientes)", "Suma", "Promedio", "Mínimo", "Máximo"]
        operacion = b_col2.selectbox("Operación", opciones_ope, key="query_ope")
        
        # Segmentar por (Opcional): -- Agrupar resultados por categorías (opcional) --
        opciones_seg = ["-- Agrupar resultados por categorías (opcional) --"] + list(df_pool.columns)
        agrupar_por_raw = b_col3.selectbox("Segmentar por (Opcional)", opciones_seg, key="query_seg")
        ...
        if var_analisis == "-- seleccionar --" or operacion == "-- seleccionar --":
            st.warning("⚠️ Debes seleccionar una Variable y una Operación válidas.")
        else:
            # Cálculo
