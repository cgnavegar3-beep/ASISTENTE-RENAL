...
with tabs[3]:
    st.markdown("### 📈 Cuadro de Mando Renal")
    df_dashboard = st.session_state["df_sync_val"]
    if not df_dashboard.empty:
        df_dashboard["FECHA_DT"] = pd.to_datetime(df_dashboard["FECHA"], format="%d/%m/%Y", errors="coerce")
        c1, c2, c3, c4 = st.columns(4)
        c1.markdown(f'<div class="db-glow-box db-blue"><div style="font-size: 0.8rem; color: #888;">TOTAL VALIDACIONES</div><div style="font-size: 2rem; font-weight: bold;">{len(df_dashboard)}</div></div>', unsafe_allow_html=True)
        c2.markdown(f'<div class="db-glow-box db-green"><div style="font-size: 0.8rem; color: #888;">PACIENTES ÚNICOS</div><div style="font-size: 2rem; font-weight: bold;">{df_dashboard["ID_REGISTRO"].nunique()}</div></div>', unsafe_allow_html=True)
        avg_afec = round(df_dashboard["Nº_TOT_AFEC_CG"].astype(float).mean(), 1)
        c3.markdown(f'<div class="db-glow-box db-red"><div style="font-size: 0.8rem; color: #888;">AFECTACIÓN MEDIA</div><div style="font-size: 2rem; font-weight: bold;">{avg_afec}</div></div>', unsafe_allow_html=True)
        count_res = len(df_dashboard[df_dashboard["RESIDENCIA"] == "Sí"])
        c4.markdown(f'<div class="db-glow-box db-purple"><div style="font-size: 0.8rem; color: #888;">PACIENTES RESIDENCIA</div><div style="font-size: 2rem; font-weight: bold;">{count_res}</div></div>', unsafe_allow_html=True)
        st.write("")
        col_g1, col_g2 = st.columns(2)
        with col_g1:
            st.markdown("#### Distribución de Función Renal (C-G)")
            fig_hist = px.histogram(df_dashboard, x="FG_CG", nbins=10, color_discrete_sequence=['#9d00ff'], labels={'FG_CG': 'FG (mL/min)'})
            st.plotly_chart(fig_hist, use_container_width=True)
        with col_g2:
            st.markdown("#### Evolución de Validaciones")
            df_evol = df_dashboard.groupby("FECHA_DT").size().reset_index(name='Conteo')
            fig_line = px.line(df_evol, x="FECHA_DT", y="Conteo", markers=True, color_discrete_sequence=['#00FF00'])
            st.plotly_chart(fig_line, use_container_width=True)
    else:
        st.info("No hay datos históricos para mostrar gráficos.")

with tabs[4]:
    st.markdown("### 🔍 Orquestador de Consulta Clínica")
    
    # --- ÁREA DE ANÁLISIS (Bloques A+B+C+D) ---
    col_analisis_1, col_analisis_2 = st.columns(2)
    
    with col_analisis_1:
        # BLOQUE A (Ahora llamado "Variable a analizar: Qué quiero medir")
        with st.container(border=True):
            st.markdown("**Variable a analizar: Qué quiero medir**")
            orq_ds = st.selectbox("Fuente de Datos", ["VALIDACIONES", "MEDICAMENTOS"], key="orq_ds")
            orq_dim = st.selectbox("Dimensión", ["CENTRO", "RESIDENCIA", "SEXO", "MEDICAMENTO", "ACCIÓN_CG"] if orq_ds == "MEDICAMENTOS" else ["CENTRO", "RESIDENCIA", "SEXO", "EDAD"], key="orq_dim")
            orq_met = st.selectbox("Métrica", ["Conteo Registros", "Pacientes Únicos", "FG_CG", "Nº_TOT_AFEC_CG", "EDAD"], key="orq_met")

        # BLOQUE B (Ahora llamado "Filtros o condiciones")
        with st.container(border=True):
            st.markdown("**Filtros o condiciones**")
            df_source = st.session_state[f"df_sync_{orq_ds.lower()}"]
            if not df_source.empty:
                col_f1, col_f2 = st.columns(2)
                with col_f1:
                    f_col = st.selectbox("Columna Filtro", df_source.columns.tolist())
                with col_f2:
                    f_val = st.multiselect("Valores", df_source[f_col].unique().tolist())
                if st.button("➕ Añadir Filtro"):
                    st.session_state.filtros_dinamicos.append((f_col, f_val))
                
                for i, (c, v) in enumerate(st.session_state.filtros_dinamicos):
                    st.caption(f"🎯 {c} in {v}")
                if st.session_state.filtros_dinamicos:
                    st.button("🗑️ Limpiar Filtros", on_change=limpiar_filtros_dinamicos)
            else:
                st.warning("Sincroniza datos en la pestaña DATOS primero.")

    with col_analisis_2:
        # BLOQUE C (Llamado "Visualización")
        with st.container(border=True):
            st.markdown("**Visualización**")
            df_final = df_source.copy()
            for c, v in st.session_state.filtros_dinamicos:
                if v: df_final = df_final[df_final[c].isin(v)]
            
            if not df_final.empty:
                st.write(f"Resultados: {len(df_final)} filas")
                st.dataframe(df_final.head(5), use_container_width=True)
            else:
                st.info("Aplica filtros para ver datos.")

        # BLOQUE D (Llamado "Ranking/Top")
        with st.container(border=True):
            st.markdown("**Ranking/Top**")
            if not df_final.empty:
                top_n = st.slider("Mostrar Top", 5, 20, 10)
                ejecutar_ranking_v29(df_final, orq_dim, orq_met, top_n, "orq")

    # --- ZONA DE SEPARACIÓN ---
    st.markdown("<br><hr style='border: 2px solid #9d00ff; opacity: 0.3;'><br>", unsafe_allow_html=True)

    # --- VENTANA DE CHAT (Zona Inferior) ---
    with st.container(border=True):
        st.markdown("**Ventana de chat (apartado de consultas rápidas)**")
        if "messages_orq" not in st.session_state:
            st.session_state.messages_orq = []
        
        for msg in st.session_state.messages_orq:
            with st.chat_message(msg["role"]):
                st.markdown(msg["content"])
        
        if prompt := st.chat_input("Pregunta sobre los datos (ej: ¿Cuál es el fármaco más afectado en Marín?)"):
            st.session_state.messages_orq.append({"role": "user", "content": prompt})
            with st.chat_message("user"):
                st.markdown(prompt)
            
            with st.chat_message("assistant"):
                contexto = f"Datos actuales: {len(df_final)} registros filtrados. Dimensión: {orq_dim}. Métrica: {orq_met}."
                respuesta = st.session_state.orq.preguntar(prompt, df_final, contexto)
                st.markdown(respuesta)
                st.session_state.messages_orq.append({"role": "assistant", "content": respuesta})

# --- AVISO LEGAL Y VERSIONADO ---
st.markdown(f'''
<div class="warning-yellow">
    <strong>⚠️ AVISO LEGAL DE RESPONSABILIDAD:</strong> Este sistema es una herramienta de apoyo y no sustituye el juicio clínico. 
    Los cálculos y sugerencias de la IA deben ser validados siempre por un profesional sanitario. 
    Usted es responsable de cualquier decisión clínica tomada.
</div>
<div style="text-align: right; font-size: 0.5rem; color: #ccc; margin-top: 10px;">v. 29.03.26 | 13:20 | Local: Marín-Grove</div>
''', unsafe_allow_html=True)
