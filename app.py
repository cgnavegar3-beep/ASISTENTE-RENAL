...
def init_session():
    defaults = {
        "active_model": "BUSCANDO...", "main_meds": "", "analisis_realizado": False,
        "resp_ia": None, "ultima_huella": "", "df_val": pd.DataFrame(), "df_meds": pd.DataFrame(),
        "df_sync_val": pd.DataFrame(), "df_sync_meds": pd.DataFrame(), "df_sync_analisis": pd.DataFrame(),
        "filtros_dinamicos": [], "soip_s": "Revisión farmacoterapéutica según función renal.",
        "soip_p": "Se hace interconsulta al MAP para valoración de ajuste posológico y seguimiento de función renal.",
        "soip_o": "", "soip_i": "", "ic_inter": "", "ic_clinica": "", "reg_id": "", "reg_centro": "", "reg_res": "-- seleccionar --"
    }
...
# --- PERSISTENCIA OPTIMIZADA (CACHÉ) ---
@st.cache_resource
def conectar_google_sheets():
    scopes = ["https://www.googleapis.com/auth/spreadsheets"]
    creds = Credentials.from_service_account_info(st.secrets["gcp_service_account"], scopes=scopes)
    return gspread.authorize(creds).open_by_key(st.secrets["GOOGLE_SHEET_ID"])

@st.cache_data(ttl=600)
def fetch_data_nube(ws_name):
    try:
        doc = conectar_google_sheets()
        ws = doc.worksheet(ws_name)
        rows = ws.get_all_values()
...
def sincronizar_todo():
    with st.spinner("Sincronizando nube..."):
        st.session_state["df_sync_val"] = fetch_data_nube("VALIDACIONES")
        st.session_state["df_sync_meds"] = fetch_data_nube("MEDICAMENTOS")
        st.session_state["df_sync_analisis"] = fetch_data_nube("ANALISIS")
        st.toast("✅ Datos actualizados", icon="🔄")

def guardar_datos_nube(df_v, df_m):
    try:
        doc = conectar_google_sheets()
        ws_v = doc.worksheet("VALIDACIONES")
        ws_m = doc.worksheet("MEDICAMENTOS")
        id_actual = st.session_state.reg_id
        
        # Validación de duplicados en nube
        existentes = ws_v.col_values(4)
        if id_actual not in existentes:
            fila_v = df_v[df_v["ID_REGISTRO"] == id_actual].iloc[-1].fillna("").values.tolist()
            ws_v.append_row(fila_v, value_input_option='USER_ENTERED')
            
            filas_m = df_m[df_m["ID_REGISTRO"] == id_actual].fillna("").values.tolist()
            if filas_m: ws_m.append_rows(filas_m, value_input_option='USER_ENTERED')
            st.cache_data.clear() # Invalida caché tras escribir
            return True
    except Exception as e:
...
def obtener_glow_class(sintesis_texto):
    ...
    return "glow-yellow" if "⚠️" in sintesis_texto else "glow-green"

def reset_meds():
    st.session_state.update({"main_meds": "", "soip_o": "", "soip_i": "", "ic_inter": "", "ic_clinica": "", "analisis_realizado": False, "resp_ia": None})
...
def inject_styles():
    ...
    .black-badge-activo { background-color: #000; color: #0f0; padding: 6px 14px; border-radius: 4px; font-family: monospace; font-size: 0.7rem; border: 1px solid #333; position: fixed; top: 10px; left: 145px; z-index: 999999; text-shadow: 0 0 5px #0f0; }
    .main-title { text-align: center; font-size: 2.5rem; font-weight: 800; color: #1E1E1E; margin-bottom: 0px; margin-top: 20px; }
    .sub-version { text-align: center; font-size: 0.6rem; color: #bbb; margin-top: -5px; margin-bottom: 20px; font-family: monospace; }
    ...
    .db-glow-box { background-color: #000; color: #fff; border: 1.5px solid #4a5568; padding: 12px; border-radius: 12px; text-align: center; margin-bottom: 10px; box-shadow: 0 0 8px #4a5568; }
    .linea-discreta-soip { border-top: 1px solid #d9d5c7; margin: 15px 0 5px 0; font-size: 0.65rem; font-weight: bold; color: #8e8a7e; text-transform: uppercase; }
    .blink-text-grabar { animation: blinker 1s linear infinite; color: #c53030; font-weight: bold; text-align: center; padding: 10px; border: 1px solid #c53030; border-radius: 5px; margin-bottom: 10px; }
    @keyframes blinker { 50% { opacity: 0; } }
    </style>
    """, unsafe_allow_html=True)
...
st.markdown(f'<div class="black-badge-activo">ACTIVO: {st.session_state.active_model}</div>', unsafe_allow_html=True)
st.markdown('<div class="main-title">ASISTENTE RENAL</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-version">v. 28 mar 2026 09:00</div>', unsafe_allow_html=True)

tabs = st.tabs(["💊 VALIDACIÓN", "📄 INFORME", "📊 DATOS", "📈 GRÁFICOS", "🔍 CONSULTA"])

# PESTAÑA 0: VALIDACIÓN
with tabs[0]:
    st.markdown("### Registro de Paciente")
    c1, c2, c3, c4, c5 = st.columns([1, 1, 1, 1.5, 0.4])
    with c1: 
        centro = st.text_input("Centro", value=st.session_state.reg_centro, key="reg_centro")
        if centro.lower() == "m": st.session_state.reg_centro = "Marín"
    with c2: st.selectbox("¿Residencia?", ["-- seleccionar --", "No", "Sí"], key="reg_res")
    with c3: st.text_input("Fecha", value=datetime.now().strftime("%d/%m/%Y"), disabled=True)
    with c4: 
        if st.session_state.reg_centro and not st.session_state.reg_id:
            st.session_state.reg_id = f"PAC-{st.session_state.reg_centro[:2].upper()}{random.randint(1000, 9999)}"
        st.text_input("ID Registro", value=st.session_state.reg_id, disabled=True)
    with c5: st.write(""); st.button("🗑️", on_click=lambda: st.session_state.update({"reg_id": "", "reg_centro": ""}))

    col_izq, col_der = st.columns(2, gap="large")
    with col_izq:
        st.markdown("#### 📋 Calculadora")
        with st.container(border=True):
            ce = st.number_input("Edad", step=1, key="calc_e", value=None)
            cp = st.number_input("Peso (kg)", key="calc_p", value=None)
            cc = st.number_input("Creatinina (mg/dL)", key="calc_c", value=None)
            cs = st.selectbox("Sexo", ["-- seleccionar --", "Hombre", "Mujer"], key="calc_s")
            fg_calc = round(((140 - ce) * cp) / (72 * cc) * (0.85 if cs == "Mujer" else 1.0), 1) if all([ce, cp, cc, cs != "-- seleccionar --"]) else 0.0
    ...
    with col_der:
        ...
        st.markdown(f'<div class="fg-glow-box"><div style="font-size: 3.2rem; font-weight: bold;">{valor_fg}</div><div style="font-size: 0.8rem; color: #9d00ff;">mL/min (C-G)</div></div>', unsafe_allow_html=True)
        st.write(""); l1, l2 = st.columns(2)
        v_mdrd = l1.number_input("MDRD-4", value=None, key="fgl_mdrd")
        v_ckd = l2.number_input("CKD-EPI", value=None, key="fgl_ckd")

    st.markdown("---")
    st.text_area("Listado de medicamentos", height=150, key="main_meds", placeholder="Pegue el listado aquí...")
    
    b1, b2 = st.columns([0.85, 0.15])
    if b1.button("🚀 VALIDAR ADECUACIÓN", use_container_width=True, type="primary"):
        if not st.session_state.main_meds or valor_fg == 0:
    ...
    if st.session_state.analisis_realizado and st.session_state.resp_ia:
        ...
        partes = [p.strip() for p in resp.split("|||") if p.strip()]
        if len(partes) >= 3:
            sintesis, tabla, detalle = partes[0], partes[1], partes[2]
            ...
            st.info(detalle)
            # Actualización de Informe SOIP
            st.session_state.soip_o = f"FG: {valor_fg} | Edad: {ce} | Crea: {cc}"
...
with tabs[1]:
    for label, key, h in [("S", "soip_s", 70), ("O", "soip_o", 70), ("I", "soip_i", 120), ("P", "soip_p", 100)]:
        st.markdown(f'<div class="linea-discreta-soip">{label}</div>', unsafe_allow_html=True)
        st.text_area(label, st.session_state[key], height=h, label_visibility="collapsed", key=f"area_{key}")

# PESTAÑA 2: DATOS
with tabs[2]:
    if st.session_state.analisis_realizado:
        st.markdown('<div class="blink-text-grabar">⚠️ DATOS PENDIENTES DE GRABAR</div>', unsafe_allow_html=True)
    if st.button("💾 GRABAR EN GOOGLE SHEETS", use_container_width=True, type="primary"):
        if guardar_datos_nube(st.session_state.df_val, st.session_state.df_meds):
...
# PESTAÑA 3: GRÁFICOS (Dashboard)
with tabs[3]:
    df_g = st.session_state["df_sync_val"]
    if not df_g.empty:
        c1, c2, c3 = st.columns(3)
        c1.metric("Pacientes", len(df_g))
        c2.metric("Media Edad", f"{df_g['EDAD'].mean():.1f}")
        c3.metric("Total Alertas", int(df_g['Nº_TOT_AFEC_CG'].sum()))
        
        st.plotly_chart(px.histogram(df_g, x="FG_CG", title="Distribución de Función Renal (C-G)"), use_container_width=True)

# PESTAÑA 4: CONSULTA DINÁMICA
with tabs[4]:
    st.markdown("### 🔍 Buscador Inteligente")
    query = st.text_input("Filtrar registros (ID, Centro, Medicamento...):")
    df_q = st.session_state["df_sync_val"]
    if query and not df_q.empty:
        df_f = df_q[df_q.astype(str).apply(lambda x: x.str.contains(query, case=False)).any(axis=1)]
        st.dataframe(df_f, use_container_width=True)

st.markdown(f'<div style="position:fixed; bottom:5px; right:15px; font-size:0.55rem; color:#888;">{st.session_state.reg_id} | v.28.03.26</div>', unsafe_allow_html=True)
