# v. 22 mar 2026 10:15 (EVO: TRADUCTOR DECIMAL ESPAÑA & ANTI-BASURA)
...
def sincronizar_desde_nube():
    try:
                ...
                for k, v in d.items():
                    if isinstance(v, str) and "," in v:
                        ...
                        try: 
                            ...
                            # FILTRO ANTI-BASURA: Si es un FG o Peso incoherente por formato
                            if k in ["FG_CG", "FG_MDRD", "FG_CKD", "PESO"] and val_num > 500:
                                val_num = val_num / 10 # Intento de corrección si se pegó sin coma
                            d[k] = val_num
                        except ValueError: pass 
                    elif isinstance(v, (int, float)):
                        # Doble check para números que Sheets ya trae como numéricos
                        if k in ["FG_CG", "FG_MDRD", "FG_CKD", "PESO"] and v > 500:
                            d[k] = v / 10
            return lista_dicts
        ...
        st.toast("✅ Nube sincronizada (Formato ES)", icon="🔄")
...
def to_spanish_num(val):
    """Convierte float de Python a string con coma para Google Sheets España"""
    ...
    try:
        ...
        if id_actual not in ids_existentes:
            ...
            for col in columnas_ordenadas:
                ...
                # EVO: Traducción a Coma para España
        ...
        for _, fila in meds_a_procesar.iterrows():
            ...
            if not ya_existe:
                fila_conv = [to_spanish_num(v.item() if hasattr(v, "item") else v) for v in fila.values.tolist()]
...
st.markdown('<div class="sub-version">v. 22 mar 2026 10:15</div>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    if not df_v_dash.empty:
        # EVO: Asegurar que los datos sincronizados se traten como números tras la traducción ES
