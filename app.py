# v. 23 mar 2026 09:30 (RESTAURACIÓN QUIRÚRGICA: TOP 5 + CROMATISMO SEMÁNTICO)
...
st.markdown('<div class="sub-version">v. 23 mar 2026 09:30</div>', unsafe_allow_html=True)
   ...
   if not df_v_dash.empty:
       ...
       with g_col1:
           ...
           if not df_m_dash.empty:
                ...
                # v. 23 mar 2026 10:45 (EVOLUCIÓN DASHBOARD: SELECTORES DINÁMICOS + TOP 5)
...
st.markdown(f'<div class="black-badge-activo">ACTIVO: {st.session_state.active_model}</div>', unsafe_allow_html=True)
st.markdown('<div class="main-title">ASISTENTE RENAL</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-version">v. 23 mar 2026 10:45</div>', unsafe_allow_html=True)
...
with tabs[3]:
    ...
    if not df_v_dash.empty:
        ...
        # EVO: ESTRUCTURA DE DOS GRÁFICOS DINÁMICOS CON SELECTORES
        g_col1, g_col2 = st.columns(2)
        
        with g_col1:
            st.markdown("##### Distribución de Riesgos")
            tipo_g1 = st.selectbox("Formato Riesgos", ["Sectores", "Barras"], key="tipo_riesgo", label_visibility="collapsed")
            if not df_m_dash.empty:
                df_m_dash["NIVEL_ADE_CG"] = pd.to_numeric(df_m_dash["NIVEL_ADE_CG"], errors='coerce').fillna(0)
                df_cat = df_m_dash.groupby("NIVEL_ADE_CG").size().reset_index(name='count')
                map_riesgos = { 0: "Sin ajuste", 1: "Precaución", 2: "Ajuste dosis", 3: "Toxicidad", 4: "Contraindicado" }
                ...
                if tipo_g1 == "Sectores":
                    fig1 = px.pie(df_cat, names="ETIQUETA", values="count", hole=0.4, color="ETIQUETA", color_discrete_map=color_map)
                else:
                    fig1 = px.bar(df_cat, x="ETIQUETA", y="count", color="ETIQUETA", text="count", color_discrete_map=color_map,
                                  category_orders={"ETIQUETA": ["Sin ajuste", "Precaución", "Ajuste dosis", "Toxicidad", "Contraindicado"]})
                
                fig1.update_layout(height=350, showlegend=True if tipo_g1=="Sectores" else False, margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
                st.plotly_chart(fig1, use_container_width=True)

        with g_col2:
            st.markdown("##### Top 5 Medicamentos")
            tipo_g2 = st.selectbox("Formato Top", ["Barras", "Sectores"], key="tipo_top", label_visibility="collapsed")
            if not df_m_dash.empty:
                ...
                if not df_alertas.empty:
                    ...
                    # Lógica de empates (rank method min)
                    if len(df_top) > 5:
                    ...
                    if tipo_g2 == "Barras":
                        fig2 = px.bar(df_top_final, x="count", y="MEDICAMENTO", orientation='h', text="count", color="count", color_continuous_scale="Reds")
                        fig2.update_layout(yaxis={'categoryorder':'total ascending'}, coloraxis_showscale=False)
                    else:
                        fig2 = px.pie(df_top_final, names="MEDICAMENTO", values="count", hole=0.3, color_discrete_sequence=px.colors.sequential.Reds_r)

                    fig2.update_layout(height=350, margin=dict(t=10, b=10, l=10, r=10), paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)')
...
# -------------------------------------
# Dashboard | Evolucionado | Implementados selectores dinámicos y estructura Dual.
# Top 5 Meds | Corregido | Lógica de empates mediante rank >= min_count.
# Cromatismo | Blindado | Mapeo semántico persistente en todos los formatos.
# Principios | Intactos | Versión v. 23 mar 2026 10:45 aplicada.
