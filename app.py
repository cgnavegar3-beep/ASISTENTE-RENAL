...
# Reemplazar con manejo de Secrets en producción
...
# Inicialización SOIP e Interconsulta
for k, v in [("soip_s", "Revisión farmacoterapéutica según función renal."), ("soip_o", ""), ("soip_i", ""), ("soip_p", "Se hace interconsulta al MAP para valoración de ajuste posológico y seguimiento de función renal."), ("ic_inter", ""), ("ic_clinica", "")]:
    if k not in st.session_state: st.session_state[k] = v

# --- CAPA DE DATOS Y CACHÉ HÍBRIDA ---
def get_gspread_service():
    creds = service_account.Credentials.from_service_account_info(GOOGLE_SERVICE_ACCOUNT, scopes=['https://www.googleapis.com/auth/spreadsheets'])
    return build('sheets', 'v4', credentials=creds)

def obtener_metadata_hash():
    """Genera un hash ligero basado en la estructura y número de filas para invalidar la caché."""
    try:
        ...
        for sheet in metadata.get('sheets', []):
            ...
            rows = sheet.get('properties', {}).get('gridProperties', {}).get('rowCount', 0)
            hash_str += f"{title}{rows}"
        return hashlib.md5(hash_str.encode()).hexdigest()
    except:
        return str(uuid.uuid4())

@st.cache_data(ttl=600)
def leer_datos_cacheado(content_hash):
    """Lee y limpia los datos de la nube. Se invalida si el hash de metadatos cambia."""
    service = get_gspread_service()
    def get_df(sheet_name):
        res = service.spreadsheets().values().get(spreadsheetId=SPREADSHEET_ID, range=f"{sheet_name}!A:Z").execute()
        vals = res.get('values', [])
        if not vals: return pd.DataFrame()
        return pd.DataFrame(vals[1:], columns=vals[0])

    return get_df("VALIDACIONES"), get_df("MEDICAMENTOS"), get_df("ANALISIS")

def sincronizar_desde_nube(force=False):
    if force: st.cache_data.clear()
    c_hash = obtener_metadata_hash()
    df1, df2, df3 = leer_datos_cacheado(c_hash)
    st.session_state["df_sync_val"] = df1
    st.session_state["df_sync_meds"] = df2
    st.session_state["df_sync_analisis"] = df3

def guardar_en_google_sheets(df_val, df_meds):
    try:
        service = get_gspread_service()
        for df, sheet in [(df_val, "VALIDACIONES"), (df_meds, "MEDICAMENTOS")]:
            body = {'values': [df.columns.tolist()] + df.values.tolist()}
            service.spreadsheets().values().append(spreadsheetId=SPREADSHEET_ID, range=f"{sheet}!A1", valueInputOption='RAW', body=body).execute()
...
st.markdown(f'<div class="black-badge-activo">ACTIVO: {st.session_state.active_model}</div>', unsafe_allow_html=True)
st.markdown('<div class="main-title">ASISTENTE RENAL</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-version">v. 28 mar 2026 08:15</div>', unsafe_allow_html=True)
...
# --- BLOQUE 2: INTERFAZ DUAL Y VALIDACIÓN ---
with tabs[0]:
    st.markdown("### Registro de Paciente")
    c1, c2, c3, c4, c5 = st.columns([1, 1, 1, 1.5, 0.4])
    def on_centro_change():
    ...
    if btn_val:
        ...
        elif faltan_datos: st.error("No se puede validar: Faltan datos obligatorios o hay selectores sin marcar.")
        else:
            huella_actual = hashlib.md5(f"{st.session_state.reg_id}{calc_e}{calc_p}{calc_c}{calc_s}{val_mdrd}{val_ckd}{st.session_state.main_meds}".encode()).hexdigest()
            if huella_actual == st.session_state.ultima_huella and st.session_state.resp_ia:
                st.toast("ℹ️ Análisis recuperado de memoria (Datos idénticos)", icon="🧠")
        ...
        try:
            ...
            try:
                ...
                if st.session_state.df_val.empty or id_actual not in st.session_state.df_val["ID_REGISTRO"].values:
                    ...
                    st.session_state.df_val = pd.concat([st.session_state.df_val, pd.DataFrame([pac_row])], ignore_index=True)
                    for m in data["medicamentos"]:
                        med_nombre = normalizar_texto_capa0(m.get("MEDICAMENTO", ""))
                        m["MEDICAMENTO"] = med_nombre
                        ya_existe_med = False
                        if not st.session_state.df_meds.empty:
                             ya_existe_med = not st.session_state.df_meds[(st.session_state.df_meds["ID_REGISTRO"] == id_actual) & (st.session_state.df_meds["MEDICAMENTO"] == med_nombre)].empty
...
with tabs[2]:
    ...
    if st.button("💾 GRABAR DATOS", type="primary", use_container_width=True):
        if not st.session_state.df_val.empty:
            guardar_en_google_sheets(st.session_state.df_val, st.session_state.df_meds)
            sincronizar_desde_nube(force=True)
            st.session_state.analisis_realizado = False; st.session_state.ultima_huella = ""
    st.write("---")
    st.markdown("### 📜 Detalle de Histórico")
    def to_excel(df):
    ...
    with sub_hist[2]: 
        ...
        if not st.session_state["df_sync_analisis"].empty:
            st.download_button(label="📥 Descargar ANÁLISIS (Excel)", data=to_excel(st.session_state["df_sync_analisis"]), file_name=f"analisis_{datetime.now().strftime('%Y%m%d')}.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    if st.button("🔄 REFRESCAR DESDE NUBE", use_container_width=True):
        sincronizar_desde_nube(force=True); st.rerun()
    ...
    if not df_v_dash.empty:
        ...
        with st.expander("🔍 Filtros Dinámicos de Análisis", expanded=True):
            ...
            with f_col1:
                centros_disp = sorted([str(x) for x in df_v_dash["CENTRO"].unique() if x])
                filtro_centro = st.multiselect("Centro", options=centros_disp)
            with f_col2: rango_edad = st.slider("Edad", 0, 110, (0, 110))
            with f_col3: rango_fg = st.slider("Filtrado Glomerular", 0.0, 150.0, (0.0, 150.0))
        mask = (df_v_dash['EDAD'].between(rango_edad[0], rango_edad[1])) & (df_v_dash['FG_CG'].between(rango_fg[0], rango_fg[1]))
        if filtro_centro: mask &= df_v_dash['CENTRO'].isin(filtro_centro)
        df_filtered_val = df_v_dash[mask]
        ids_filtrados = df_filtered_val["ID_REGISTRO"].unique()
        df_filtered_meds = df_m_dash[df_m_dash["ID_REGISTRO"].isin(ids_filtrados)]
        df_anal_sync = st.session_state.get("df_sync_analisis", pd.DataFrame())
        try: total_pacientes = int(df_anal_sync.iloc[0, 1]) if not df_anal_sync.empty else df_filtered_val["ID_REGISTRO"].nunique()
        except: total_pacientes = df_filtered_val["ID_REGISTRO"].nunique()
        total_meds_revisados = df_filtered_val["Nº_TOTAL_MEDS_PAC"].sum()
        afectados_total = int(df_filtered_val["Nº_TOT_AFEC_CG"].sum())
        porcentaje_afec = (afectados_total / total_meds_revisados * 100) if total_meds_revisados > 0 else 0
        try: pac_afectados_pct = df_anal_sync.iloc[10, 1] if not df_anal_sync.empty else "0%"
        except: pac_afectados_pct = "0%"
        kpi_c1, kpi_c2, kpi_c3, kpi_c4 = st.columns(4)
        with kpi_c1: st.markdown(f'<div class="db-glow-box db-blue"><div style="font-size: 0.75rem; color: #BBBBBB;">Pacientes Revisados</div><div style="font-size: 1.8rem; font-weight: bold; color: #FFFFFF;">{total_pacientes}</div></div>', unsafe_allow_html=True)
        with kpi_c2: st.markdown(f'<div class="db-glow-box db-green"><div style="font-size: 0.75rem; color: #BBBBBB;">Total medicamentos revisados</div><div style="font-size: 1.8rem; font-weight: bold; color: #FFFFFF;">{total_meds_revisados}</div></div>', unsafe_allow_html=True)
        with kpi_c3: st.markdown(f'<div class="db-glow-box db-red"><div style="font-size: 0.75rem; color: #BBBBBB;">Alertas Detectadas (Totales)</div><div style="font-size: 1.8rem; font-weight: bold; color: #FFFFFF;">{afectados_total} <span style="font-size: 0.9rem; color: #feb2b2;">({porcentaje_afec:.1f}%)</span></div></div>', unsafe_allow_html=True)
        with kpi_c4: st.markdown(f'<div class="db-glow-box db-purple"><div style="font-size: 0.75rem; color: #BBBBBB;">% de medicamentos afectados</div><div style="font-size: 1.8rem; font-weight: bold; color: #FFFFFF;">{pac_afectados_pct}</div></div>', unsafe_allow_html=True)
        g_col1, g_col2 = st.columns(2)
        with g_col1:
            st.markdown("##### Distribución de Riesgos")
            if not df_filtered_meds.empty:
                ...
                df_cat["ETIQUETA"] = df_cat["NIVEL_ADE_CG"].map(map_riesgos)
                tipo_graf_riesgo = st.selectbox("Visualización", ["-- seleccionar --", "Sectores", "Barras H", "Barras V"], key="sel_riesgo")
                if tipo_graf_riesgo in ["-- seleccionar --", "Sectores"]:
                    fig_riesgo = px.pie(df_cat, names="ETIQUETA", values="count", color="ETIQUETA", color_discrete_map=color_map, hole=0.4)
                    fig_riesgo.update_layout(height=300, margin=dict(t=10, b=10, l=40, r=10), showlegend=True, legend=dict(orientation="v", yanchor="middle", y=0.5, xanchor="left", x=1.05))
                    st.plotly_chart(fig_riesgo, use_container_width=True)
                elif tipo_graf_riesgo == "Barras H":
        ...
        with g_col2:
            ...
            if not df_filtered_meds.empty:
                ...
                if not df_alertas.empty:
                    ...
                    df_top['Rank'] = df_top['Frecuencia'].rank(method='min', ascending=False)
                    df_top_final = df_top[df_top['Rank'] <= 5].sort_values(by="Frecuencia", ascending=False)
                    if tipo_graf_top in ["-- seleccionar --", "Barras Horizontales"]:
                        fig_top = px.bar(df_top_final, y="MED_NORM", x="Frecuencia", orientation='h', text="Frecuencia", color="Frecuencia", color_continuous_scale="Reds")
                        fig_top.update_layout(showlegend=False, height=300, margin=dict(t=10, b=10, l=10, r=10), yaxis={'categoryorder':'total ascending'})
                        st.plotly_chart(fig_top, use_container_width=True)
                    elif tipo_graf_top == "Barras Verticales":
...
with tabs[4]:
    ...
    if not df_pool.empty:
        with st.container(border=True):
            ...
            if col_a2.button("🗑️ Limpiar Filtros"): limpiar_filtros_dinamicos(); st.rerun()
            for i, filtro in enumerate(st.session_state.filtros_dinamicos):
                ...
                if "contiene" in filtro["op"]: filtro["val"] = f_c3.text_input(f"Valor {i+1}", key=f"f_val_{fid}", value=filtro["val"])
                elif filtro["col"] in ["EDAD", "FG_CG", "Nº_TOTAL_MEDS_PAC", "PESO", "CREATININA", "NIVEL_ADE_CG", "Nº_TOT_AFEC_CG"]:
                    ...
                    filtro["val"] = f_c3.multiselect(f"Valores {i+1}", opciones_unicas, key=f"f_val_{fid}")
            ...
            try:
                ...
                if "==" in f["op"]:
                    if isinstance(f["val"], list) and f["val"]: 
                        df_filtered_query = df_filtered_query[col_data.astype(str).apply(normalizar_texto_capa0).isin([normalizar_texto_capa0(v) for v in f["val"]])]
                    elif f["val"] != "": df_filtered_query = df_filtered_query[col_data.apply(lambda x: normalizar_texto_capa0(str(x))) == normalizar_texto_capa0(f["val"])]
                ...
                elif "contiene" in f["op"]: df_filtered_query = df_filtered_query[col_data.apply(lambda x: normalizar_texto_capa0(str(x))).str.contains(normalizar_texto_capa0(f["val"]), case=False, na=False)]
            except: continue
        ...
        if var_analisis == "-- seleccionar --" or operacion == "-- seleccionar --": st.info("Configura la variable y operación.")
        else:
            ...
            if agrupar_por == "Ninguno":
                if "Total" in operacion: resultado = len(df_filtered_query[var_analisis])
                    ...
                    vec = pd.to_numeric(df_filtered_query[var_analisis], errors='coerce')
                    resultado = vec.sum() if "Suma" in operacion else vec.mean() if "Promedio" in operacion else vec.max()
                ...
                try:
                    ...
                    if formato_salida == "KPI": st.metric("Registros en Cohorte", len(df_filtered_query))
                        ...
                        df_h = df_filtered_query.copy(); df_h[var_analisis] = pd.to_numeric(df_h[var_analisis], errors='coerce')
                        if "FG" in var_analisis:
                            bins = [-float('inf'), 15, 30, 45, 60, 90, float('inf')]; labels = ['< 15 (G5)', '15-29 (G4)', '30-44 (G3b)', '45-59 (G3a)', '60-89 (G2)', '≥ 90 (G1)']
                            df_h['BIN'] = pd.cut(df_h[var_analisis], bins=bins, labels=labels, right=False)
                            fig = px.histogram(df_h, x='BIN', color_discrete_sequence=['#9d00ff'], category_orders={"BIN": labels})
                        elif var_analisis == "EDAD":
                            bins = [-float('inf'), 50, 61, 71, 81, 91, float('inf')]; labels = ['< 50a', '50-60a', '61-70a', '71-80a', '81-90a', '> 90a']
                            df_h['BIN'] = pd.cut(df_h[var_analisis], bins=bins, labels=labels, right=False)
                            fig = px.histogram(df_h, x='BIN', color_discrete_sequence=['#9d00ff'], category_orders={"BIN": labels})
                        else: fig = px.histogram(df_h, x=var_analisis, color_discrete_sequence=['#9d00ff'], marginal="box")
                        fig.update_layout(bargap=0.1); st.plotly_chart(fig, use_container_width=True)
                except: st.warning("Error en el cálculo.")
        st.markdown("---")
        with st.expander("📄 Ver Datos Crutos"): st.dataframe(df_filtered_query, use_container_width=True)
...
st.markdown(f'<div style="text-align: right; font-size: 0.6rem; color: #ccc; font-family: monospace;">v. 28 mar 2026 08:15</div>', unsafe_allow_html=True)
